...
def save_data_scheme_close(conf_dic, scheme):
	...
	scheme.write("\nwhole error: " + str(conf_dic['whole_error']) + "; " + str(round(100 - (conf_dic['whole_error'] / 255 * 100), 2)) + "% accuracy")
	scheme.close()
...
def generate(conf_dic):
	...
	while (max_con_flag and conn_count < max_conns) or (not max_con_flag and new_error <= whole_error):
		...
		elif (conn_count % skip_error_check == 0):
			new_error = get_whole_error(image, result)
		else:
			new_error = whole_error - 0.0001
		prev_pin = cur_pin
...
def main():
	...
	if (conf_dic['if_show']):
		show_tk_img(root, canvas, conf_dic['image'], canvas_size)
